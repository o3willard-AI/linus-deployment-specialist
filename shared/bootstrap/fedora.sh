#!/usr/bin/env bash
# =============================================================================
# Linus Deployment Specialist - Fedora Bootstrap Script (Ephemeral VM)
# =============================================================================
# Purpose: Initial OS-level setup for Fedora Workstation/Ephemeral
...
#
# Note: This script is designed for ephemeral VMs using Fedora Workstation
# images as a base template in Proxmox/LXC environments.
#
...
#   FEDORA_VER          - Fedora version (default: 40)
#
# Usage:
#   ./fedora.sh
#   FEDORA_VER=39 ./fedora.sh
#
# Exit Codes:
#   0 - Success
#   1 - General error
...
readonly FEDORA_VER="${FEDORA_VER:-40}"

# Package lists - base packages for Fedora (different from RHEL-based distros)
...
# Fedora-specific development packages (optional extras)
readarray -T FEDEV_PACKAGES < <(cat <<EOF
devtoolset-10-gcc
devtoolset-10-gcc-c++
devtoolset-10-make
python3-devel
...
install_packages() {
    ...
    # Fedora uses different repo structure
    log_info "Configuring Fedora repositories..."

    # Configure base repository for local installation (Fedora specific)
    cat <<EOF > /etc/yum.repos.d/linus-base.repo
[linus-fedora]
name=Linus Deployment Fedora Repository
baseurl=https://mirrors.fedoraproject.org/releases/${FEDORA_VER}/x86_64/os/\
enabled=1
gpgcheck=0
EOF

    log_info "Using Fedora ${FEDORA_VER} repository"
    log_info "Base repository: ${BASE_URL}"
...
configure_system() {
    ...
    # Configure Fedora specific settings (firewalld, selinux)
    log_info "Configuring Fedora environment..."

    # Set firewalld service if not already running
    systemctl enable --now firewalld 2>/dev/null || true

    # Configure default firewall zones
    firewall-cmd --permanent --direct --add-rule ipv4 filter INPUT 0 \
        -p tcp -m state --state NEW -m tcp --dport 22 -j ACCEPT 2>/dev/null || true
...
install_optional_packages() {
    ...
    # Install Fedora-specific development tools
    log_info "Installing development and cloud management packages..."
    
    dnf -y install "${FEDEV_PACKAGES[@]}" || {
...
verify_installations() {
    ...
    # Verify Fedora identity
    if [[ -f /etc/fedora-release ]]; then
        local fedora_version=$(cat /etc/fedora-release | grep -oP '\d+(\.\d+)*')
        log_info "Fedora version: $fedora_version detected"
    fi
...
main() {
    log_header "Fedora Bootstrap (Ephemeral VM)"
    log_info "Version: 1.2 (Enhanced with network fallback support)"
    log_info "Fedora Version: ${FEDORA_VER}"
    ...
    log_success "Fedora bootstrap complete!"
}
