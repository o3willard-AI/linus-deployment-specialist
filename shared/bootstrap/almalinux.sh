...
# Usage:
#   ./almalinux.sh
#   TIMEZONE=America/New_York ./almalinux.sh
#
# Exit Codes:
#   0 - Success
#   1 - General error
...
validate_environment() {
    ...
    if [[ "${ID}" != "almalinux" ]]; then
        log_error "This script is for AlmaLinux only (detected: ${ID})"
        return 2
    fi
...
main() {
    log_header "Linus AlmaLinux Bootstrap"
    ...
    log_success "AlmaLinux bootstrap completed successfully"
    return 0
}
