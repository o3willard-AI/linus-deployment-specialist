#!/usr/bin/env bash
# =============================================================================
# Linus Deployment Specialist - Oracle Linux 9 Bootstrap Script
# =============================================================================
# Purpose: Initial OS-level setup for Oracle Linux 9.x
...
# Usage:
#   ./ol9.sh
#   TIMEZONE=America/New_York ./ol9.sh
#
# Exit Codes:
#   0 - Success
#   1 - General error
...
# Package lists - base packages for Oracle Linux 9 (compatible with AlmaLinux/Rocky)
...
# Oracle Linux specific packages (optional)
readarray -t OL_PACKAGES < <(cat <<EOF
oracle-java17-sdk
oracle-ulimits
pcre2-utils
git-core
...
install_packages() {
    ...
    # Oracle Linux uses separate kernel repo - configure repos first
    log_info "Configuring Oracle Linux repositories..."

    # Enable OL9 repo if not already enabled
    if command -v dnf &> /dev/null && [[ -f /etc/yum.repos.d/*.ol9* ]]; then
        log_info "Oracle Linux 9 repositories detected"
        
        # Configure base repository for local installation
        cat <<EOF > /etc/yum.repos.d/linus-base.repo
[linus-base]
name=Linus Deployment Base Repository
baseurl=${BASE_URL:-https://yum.oracle.com/repo/OL9/ol-9/x86_64/public/}
enabled=1
gpgcheck=0
EOF
        ...
        # Fallback to EPEL or Fedora repos if OL repos not found
        log_warn "Standard Oracle Linux repos not detected, using generic repos"
        
        cat <<EOF > /etc/yum.repos.d/linus-base.repo
[linus-base]
name=Linus Deployment Base Repository
baseurl=https://mirrors.kernel.org/fedora/epel/9/x86_64/
enabled=1
gpgcheck=0
EOF

        log_info "Using fallback repository: ${BASE_URL}"
    fi
...
configure_system() {
    ...
    if [[ "$TIMEZONE" != "UTC" ]]; then
        ...
        chrony-setshamtodriftinterval 10 2>/dev/null || true
    ...
    # Configure Oracle Linux specific settings
    log_info "Configuring Oracle Linux environment..."

    # Add Oracle-specific environment variables
    cat <<EOF >> /etc/profile.d/ol9-env.sh
#!/bin/bash
# Oracle Linux 9 Environment Variables
export OL_DEBUG_MODE=0
export OL_HOSTNAME=$HOSTNAME
EOF

    chmod +x /etc/profile.d/ol9-env.sh
...
install_optional_packages() {
    ...
    # Install Oracle Linux specific packages
    log_info "Installing Oracle-specific packages..."
    
    dnf -y install "${OL_PACKAGES[@]}" || {
        log_warn "Some Oracle packages may have failed to install"
    }
...
verify_installations() {
    ...
    # Verify Oracle Linux identity
    if [[ -f /etc/oracle-release ]]; then
        local ol_version=$(cat /etc/oracle-release | grep -oP '\d+(\.\d+)*')
        log_info "Oracle Linux version: $ol_version detected"
    fi
...
main() {
    log_header "Oracle Linux 9 Bootstrap"
    ...
    log_success "Oracle Linux 9 bootstrap complete!"
}
