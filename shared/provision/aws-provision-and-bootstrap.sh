#!/usr/bin/env bash
# =============================================================================
# Linus Deployment Specialist — AWS EC2 Provision + Bootstrap Driver
# =============================================================================
...
#
# Architecture: set AWS_ARCHITECTURE=arm64 for Graviton instances.
#
# Exit Codes:
...
AWS_ARCHITECTURE="${AWS_ARCHITECTURE:-}"         # x86_64 or arm64 (auto-detected if unset)
AWS_INSTANCE_TYPE="${AWS_INSTANCE_TYPE:-}"        # Auto-selected from CPU/RAM if unset
AWS_AMI_ID="${AWS_AMI_ID:-}"                      # Auto-detected from VM_OS_TYPE if unset
AWS_AMI_FALLBACKS="${AWS_AMI_FALLBACKS:-}"
...
# Paths
readonly PROVISION_SCRIPT="$SCRIPT_DIR/aws.sh"
...
#       PARSED_VM_NAME, PARSED_VM_OS_TYPE, PARSED_VM_AMI_ID,
#       PARSED_VM_INSTANCE_TYPE, PARSED_VM_ARCHITECTURE, PARSED_COST
parse_linus_result() {
    local output="$1"
    local key val
    ...
    while IFS=':' read -r key val; do
        ...
        case "$key" in
            ...
            VM_AMI_ID)        PARSED_VM_AMI_ID="$val" ;;
            VM_INSTANCE_TYPE) PARSED_VM_INSTANCE_TYPE="$val" ;;
            VM_ARCHITECTURE)  PARSED_VM_ARCHITECTURE="$val" ;;
...
run_provision() {
    log_header "Phase 1: EC2 Instance Provisioning"

    log_info "Provisioning ${VM_OS_TYPE} instance in ${AWS_REGION} (arch: ${AWS_ARCHITECTURE:-auto})..."

    local provision_output provision_ec=0

    # Run provision with all required env vars
    provision_output=$(AWS_REGION="$AWS_REGION" \
        AWS_KEY_NAME="$AWS_KEY_NAME" \
    ...
    log_success "Provisioned: ${PARSED_VM_ID} @ ${PARSED_VM_IP} (${PARSED_VM_USER:-ubuntu})"
}
...
" 2>/dev/null || echo "${total_seconds}s")
    ...
    log_info "  Instance ID:    ${PARSED_VM_ID:-N/A}"
    log_info "  Instance IP:    ${PARSED_VM_IP:-N/A}"
    log_info "  SSH:            ${PARSED_VM_USER:-ubuntu}@${PARSED_VM_IP:-N/A}"
    log_info "  OS type:        ${PARSED_VM_OS_TYPE:-${VM_OS_TYPE}}"
    log_info "  Instance type:  ${PARSED_VM_INSTANCE_TYPE:-N/A}"
    log_info "  Architecture:   ${PARSED_VM_ARCHITECTURE:-N/A}"
    log_info "  AMI:            ${PARSED_VM_AMI_ID:-N/A}"
    ...
    # Output structured result for downstream consumers (unified provider contract)
    echo ""
    ...
    echo "LINUS_VM_AMI_ID:${PARSED_VM_AMI_ID:-}"
    echo "LINUS_VM_INSTANCE_TYPE:${PARSED_VM_INSTANCE_TYPE:-}"
    echo "LINUS_VM_ARCHITECTURE:${PARSED_VM_ARCHITECTURE:-}"
...
main() {
    log_header "Linus AWS EC2 — Provision + Bootstrap Pipeline"
